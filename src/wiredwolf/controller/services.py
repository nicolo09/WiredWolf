...
class ServiceManager:
    ...
    def register_service(self, name: str, receiverSocket: socket.socket) -> ServiceInfo:
        self._port = receiverSocket.getsockname()[1]
        self.__logger.info(f"Registering service {name} on port {self._port}...")
    ...
    def connect_to_service(self, service_name: str) -> socket.socket:
        ...
        if service_info:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Could not connect to service {service_name}."
                ) from e  # TODO: Change RuntimeError with something more appropriate
        else:
